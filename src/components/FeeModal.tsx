...
const FeeModal: React.FC<FeeModalProps> = ({ isOpen, onClose, onSave, fee, theme }) => {
  ...
  const uniqueApartments = Array.from(
    new Set(residents.map(r => r.apartmentId))
  );
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const owner = residents.find(
                    r => r.apartmentId === aptId && r.role === 'owner'
                  );
                ...
                className={`w-full px-5 py-3 border rounded-2xl focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all ${
              ...
              <div className="flex gap-2">
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={handleChange}
                    className={`w-4/7 px-5 py-3 border rounded-2xl ${
                  ...
                  <select
                    ...
                    value={formData.year || ''}
                    onChange={handleChange}
                    className={`w-3/7 px-5 py-3 border rounded-2xl ${
                      isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200'
                    }`}
                  >
            ...
            {[
              ...
              <div key={field.name} className="space-y-2">
                ...
                <input
                  name={field.name}
                  type="number"
                  value={formData[field.name as keyof FeeItem] ?? ''}
                  onFocus={(e) => {
                  ...
                  onBlur={(e) => {
                    if (e.target.value === '') {
                      handleChange({
                        target: { name: field.name, value: '0' }
                      } as any);
                    }
                  }}
                  onChange={handleChange}
                  className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm font-bold ${
                    isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-800'
                  }`}
                />
