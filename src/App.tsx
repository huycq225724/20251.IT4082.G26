  ...
  return (
    <div className={`flex min-h-screen ${theme === 'dark' ? 'bg-slate-950' : 'bg-slate-50'} transition-colors duration-300`}>
          ...
          <div className="flex items-center gap-3">
            {activeTab !== 'dashboard' && activeTab !== 'profile' && (
              <div className="relative group">
                ...
                />
                <svg
                  className="w-5 h-5 absolute left-3 top-2.5 text-slate-400 group-focus-within:text-blue-500 transition-colors"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            {activeTab === 'residents' && (
              <button
                ...
                className="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-900/20 flex items-center gap-2"
              >
      ...
      <ResidentModal
        ...
        onSave={(resident: Resident) => {
          if (editingResident) {
            const newList = residents.map(r =>
